...
<body onload="result()">
	...
	echo "<div style='float:right' class='scrl'><button class='btn-danger'><i class='fa fa-exclamation-triangle my'></i> Remaining Chances : ".$clicks."</button></div>";
	...
	<hr>
		...
		<span style='width:100%;border:1px solid white;border-radius:100px;'><b id="timer">
	<div style="width:100%;background-color:green;height:50px;border-radius:100px;background-color:green;"></div></b></span><br>
			
	<center><input type="text" name="symbol" id="usrinput" class="btn btn-default" style="height:400px;width:30%;font-size:150px;color:teal;font-family:arial;">
...
<div style="position:absolute;right:10%;bottom:5%;"><i style="float:right;margin-right:50px;color:green;">&copy; Venkatesh Mogili,N130010</i></div>
		<?php
		}
	}
}
...
<style>
body{display:none;background:url('img/b1.jpg');background-repeat:no-repeat;background-size:100% 100%;background-attachment:fixed;}
/*position:relative;-webkit-animation: myb 1s 1;-webkit-animation-play-state: play;animation: myb 1s 1;animation-play-state: play;}
...
*/
.scrl{position:relative;-webkit-animation: myb 1s 1;-webkit-animation-play-state: play;animation: myb 1s 1;animation-play-state: play;}
@-webkit-keyframes myb {from {left:30px;background-color:black;-webkit-transform:rotateY(-360deg);transform:rotateY(-360deg);-moz-transform:rotateY(-360deg);}to {left:1px;}}
@-moz-keyframes myb {from {left:30px;-webkit-transform:rotateY(-360deg);transform:rotateY(-360deg);-moz-transform:rotateY(-360deg);}to {left:1px;}}
@keyframes myb {from {left:30px;-webkit-transform:rotateY(-360deg);transform:rotateY(-360deg);-moz-transform:rotateY(-360deg);}to {left:1px;}}
}
...
setInterval(function(){
	...
	if(x==5)
	{
	y.innerHTML="<div style='width:100%;height:50px;border-radius:100px;background-color:#006600;transition:0.7s;' id='full'></div>";
	}
	if(x==4)
	{
	y.innerHTML="<div style='width:80%;height:50px;border-radius:100px;background-color:#006633;transition:0.7s;'></div>";
	}
	if(x==3)
	{
	y.innerHTML="<div style='width:60%;height:50px;border-radius:100px;background-color:#006666;transition:0.7s;'></div>";
	}
	if(x==2)
	{
	y.innerHTML="<div style='width:40%;height:50px;border-radius:100px;background-color:#CC6600;transition:0.7s;'></div>";
	}
	if(x==1)
	{
	y.innerHTML="<div style='width:20%;height:50px;border-radius:100px;background-color:#CC3300;transition:0.7s;'></div>";
	}
	if(x==0)
	{
	y.innerHTML="<div style='width:0%;height:50px;border-radius:100px;background-color:#CC0000;transition:0.7s;'></div>";
	}
	if(x==1){
	x=6;
	}
...
<style>
.full{width:100%;height:50px;border-radius:100px;background-color:#006600;transition:0.7s;}
</style>